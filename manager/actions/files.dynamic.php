...
function deleteFolder (folder) {
	if (confirmDeleteFolder()) 
		window.location.href=	
		 "index.php?a=31&mode=deletefolder&path="+current_path+"&folderpath="+current_path+'/'+folder;
	return false;
}
...
function deleteFile(file) {
	if (confirmDelete()) 
		window.location.href=
		"index.php?a=31&mode=delete&path="+current_path+'/'+file;
	return false;
}
</script>
<?php
    ...
    while ($file = $dir->read()) {
        if(!in_array($file, $excludes)) {
            ...
            if(is_dir($newpath)) {
                ...
                $dirs_array[$dircounter]['delete'] = is_writable($curpath) ? '<span style="width:20px"><a href="javascript: deleteFolder(\''.$file.'\');"><img src="media/style/'.$manager_theme.'images/icons/delete.gif" alt="'.$_lang['file_delete_folder'].'" title="'.$_lang['file_delete_folder'].'" /></a></span>' : '';
